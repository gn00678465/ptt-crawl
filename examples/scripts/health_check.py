    ...
    async def check_firecrawl_health(self) -> HealthStatus:
        ...
        try:
            ...
            return HealthStatus(
                component="firecrawl",
                status="critical",
                message=f"Firecrawl API 檢查失敗: {str(e)}",
...
def print_health_report(health_data: Dict[str, Any], detailed: bool = False):
    ...
    print(f"整體狀態: {get_status_emoji(health_data['overall_status'])} {health_data['overall_status'].upper()}")
