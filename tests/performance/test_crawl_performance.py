...
class TestCrawlServicePerformance:
    ...
    def crawl_service_fast(self, mock_fast_dependencies):
        ...
        mock_fast_dependencies["firecrawl_service"].scrape_board_page = AsyncMock(
            return_value=mock_response
        )
        mock_fast_dependencies["firecrawl_service"].scrape_article_with_retry = AsyncMock(
            return_value=mock_response
        )
    ...
    @pytest.mark.asyncio()
    async def test_single_page_crawl_performance(
        self, crawl_service_fast: CrawlService, performance_monitor: PerformanceMonitor
    ):
    ...
    @pytest.mark.asyncio()
    async def test_multiple_pages_crawl_performance(
        self, crawl_service_fast: CrawlService, performance_monitor: PerformanceMonitor
    ):
    ...
    @pytest.mark.asyncio()
    async def test_concurrent_crawl_performance(
        self, crawl_service_fast: CrawlService, performance_monitor: PerformanceMonitor
    ):
        ...
        tasks = [crawl_service_fast.crawl_board(f"Board{i}", pages=2) for i in range(3)]
    ...
    @pytest.mark.asyncio()
    async def test_large_article_list_performance(
        self,
        crawl_service_fast: CrawlService,
        performance_monitor: PerformanceMonitor,
        mock_fast_dependencies,
    ):
        ...
        mock_fast_dependencies[
            "firecrawl_service"
        ].extract_article_links.return_value = large_article_list
...
class TestFirecrawlServicePerformance:
    ...
    def test_article_link_extraction_performance(self, firecrawl_service: FirecrawlService):
        ...
        response_data = {"data": {"markdown": markdown_content, "links": []}}
    ...
    def test_content_cleaning_performance(self, firecrawl_service: FirecrawlService):
        ...
        print(
            f"Content cleaning performance: {len(dirty_content)} -> {len(cleaned_content)} chars in {elapsed_time:.3f}s"
        )
    ...
    def test_category_filtering_performance(self, firecrawl_service: FirecrawlService):
        ...
        links = [
            ...
            if i % 3 == 0
            else {"text": f"[標的] Article {i}", "category": "標的", "url": f"url_{i}"}
            if i % 3 == 1
            else {"text": f"[請益] Article {i}", "category": "請益", "url": f"url_{i}"}
        ...
        print(
            f"Category filtering performance: {len(links)} -> {len(filtered)} links in {elapsed_time:.3f}s"
        )
...
class TestDatabasePerformance:
    ...
    async def test_batch_insert_performance(self):
        ...
        print(
            f"Batch data preparation performance: {len(articles)} articles in {elapsed_time:.3f}s"
        )
